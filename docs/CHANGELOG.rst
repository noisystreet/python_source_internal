...
* 全书 75 个 RST 锚点（ ``.. _objects-pyobject:`` 等），实现跨章 :ref: 交叉引用
* 所有章节目录统一添加 ``:ref:`` 链接，形成章节间引用网络
* ARCHITECTURE、builtins、deferred-annotations 三章补充参考资料段落

修复
^^^^

* 全书 63 个"参考资料"段落统一风格（:ref: / :pep: / :file: 三类条目齐备）
* RST 内联标记与 CJK 字符紧邻导致粗体/代码渲染为字面文本的 bug
  — 批量修复 58 个文件中 600+ 处 CJK 与 ``**bold**`` / ``````code`````` 之间的空格
* typeobject.rst 中 2 处 CJK 标题下划线宽度不足
